...
if st.session_state.page == 'portal':
    ...
    with col1:
        ...
        if st.button("Prepare a story pitch", type="primary", use_container_width=True):
            ...
            st.rerun() # FIX: Force immediate rerun to prevent double-click issue
...
# --- Page 2: The Questionnaire ---
    ...
    with st.form("pitch_form"):
        ...
        # NOTE: For this version, we are locking into the Socratic dialogue to test the V8 logic
        response_mode = "Socratic dialogue"
...
# --- Page 3: The Prompt Recipe ---
    ...
    st.markdown("This prompt contains our new V8 'Editorial Reaction' logic. Copy the text below to test it with your preferred AI.")
    st.markdown("---")

    data = st.session_state.get('form_data', {})

    # Build the CONTEXT block from all available (non-empty) data
    context_lines = [
    ...
    # --- V8 PROMPT with "Editorial Reaction" logic ---
    ...
    * **Pattern C: Skeptical but open...** (Use for pitches that feel more like a broad topic than a focused story).
    ...
    ## Turn 4+: Targeted Coaching
    After you have a clear understanding of the user's thinking from the Deep Dive, you can shift to providing actionable advice.
    - Offer specific next steps (e.g., "Based on that, here are two reporting actions you could take...").
    - Focus on reporting actions, sourcing ideas, and verification plans, not writing solutions.
    ...
    st.subheader("Your Assembled V8 Prompt")
    st.code(final_prompt, language="markdown")
    
    # FIX: A more reliable copy button implementation
    if st.button("Copy Prompt to Clipboard"):
        st.components.v1.html(
            f"""
            <textarea id="prompt-text" style="opacity: 0; position: absolute;">{final_prompt}</textarea>
            <script>
